...
const Index = () => {
  ...
  const [session, setSession] = useState<LiveSessionState>(liveSession.getState());
  ...
  const prevLenRef = useRef<number>(segments.length);
  ...
  useEffect(() => {
    const unsub = liveSession.subscribe(setSession);
    return () => unsub();
  }, []);
  ...
  function toggleSelect(id: string) {
    liveSession.toggleSelect(id);
  }

  function clearSelection() {
    liveSession.clearSelection();
  }
            ...
            <div className="flex items-center gap-3">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  {connected ? (
                    <Button variant="destructive" size="sm" onClick={() => liveSession.disconnect()}>
                  ...
                  <TabsContent value="factcheck" className="px-4 pt-4 pb-0 space-y-3">
                    ...
                    <p className="text-xs text-muted-foreground">
                     ...
                     <SpeakersForm
                       ...
                       onRename={({ from, to }) => {
                         liveSession.renameSpeaker(from, to);
                       }}
                       aiRuns={runs}
                     />
        ) : (
